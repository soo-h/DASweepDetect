...
## usage: python3.8 path1 paht2 name 
dataSet_train,y,dataSet_val,y_val,dataSet_test,y_test,target_data,model_name = sys.argv[1:9]
...
if os.path.isdir(target_data):
    ...
    target_data = [dirpath + name for name in os.listdir(dirpath)] 
    if len(target_data) != np.sum([True for n in target_data if n.endswith('npy') ]):
        print("Error:The files in the input folder must be feature map of vcf file")
        sys.exit(1)
...
#BUFFER_SIZE_TEST = len(dataSet_test)
...
target_dataset = tf.data.Dataset.from_tensor_slices((target_data,np.repeat(1.,len(target_data)))).shuffle(BUFFER_SIZE_TARGET).batch(BATCH_SIZE)
...
def train_step(x_s,x_t,alpha):
    ...
    #source_domain_label = np.random.uniform(0,0.3,len(class_label))
    ...
    target_X,_ = x_t
    #—— = np.random.uniform(0.7,1,len(target_X))
    ...
    with tf.GradientTape() as tape:
        ...
        total_loss = err_s_label + 0.5*(err_s_domain + err_t_domain)
...
f = open(f'{model_name}.log','w')
DANN_name = f'DANN_{model_name}'
### 早停设置
stop_monitor = early_stop(5)
...
for epoch in range(EPOCHS):
    ...
    while i < n_step:
        ...
        total_loss,class_loss,err_s_domain,err_t_domain = train_step(x_s,x_t,alpha)
        
        if total_loss == class_loss == err_s_domain == err_t_domain:
            sys.exit('nan')
    ...
    template = ("Epoch {}, TotalLoss: {}, Loss_Class: {}, Loss_Domain:{},Accuracy: {} \n validSet: Loss1: {},Accuracy_val: {}")
    epoch_log = template .format(epoch+1,train_loss_total,train_loss_container.result(),domain_loss_container.result(),train_accuracy.result()*100,\
    ...
    if stop == True:
        f.write(f'best model:  {DANN_name}_epoch{best_iter-1}')
        f.close()
        break
...
f.close()
###########输出模型在测试集上的评估结果：混淆矩阵及训练曲线

with open(f'{model_name}.log','r') as log:
    train_log = []
    valid_log = []
    for line in log:
...
def acc_loss_plot(train_acc,train_loss,valid_acc,valid_loss):
    epochs = range(len(valid_loss))
    fig,ax = plt.subplots(1,2,figsize = (20,6))
    ax[0].plot(epochs,train_acc, 'r', label='Training accuracy')
    ax[0].plot(epochs,valid_acc, 'b', label='validation accuracy')
    ax[0].set_title('Training and validation accuracy')
    ax[0].legend(loc='lower right')

    ax[1].plot(epochs, train_loss, 'r', label='Training loss')
    ax[1].plot(epochs, valid_loss, 'b', label='validation loss')
    ax[1].set_title('Training and validation loss')
    ax[1].legend()
    fig.savefig('train_acc_loss.png')
    return 0

acc_loss_plot(train_acc,train_loss,valid_acc,valid_loss)

#model =tf.keras.models.load_model({DANN_name}_epoch{best_iter-1})

#pred,_ = model(test_dataset,1.)
#pred = np.argmax(pred,axis=1)
#conf_mx = confusion_matrix(pred,y_test)
